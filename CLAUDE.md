...
- **Reconciliation is by run token, and it deletes.** Every post and term the run touches is stamped with `SYNC_RUN_META` (`tg_sync_run`); finalize removes what is *not* stamped. That is how items deleted in TapGoods finally disappear (measured on a live site: 8,414 stale posts removed on the first completed run, 26,596 → 18,182). Deletion is permanent (`wp_delete_post` with force), so **both cleanups are guarded** and those guards must not be weakened: terms refuse to reconcile a taxonomy the run stamped nothing in, and items require the stamped set to be at least `cleanup_min_stamped_ratio()` (0.5, override `TG_SYNC_CLEANUP_MIN_RATIO`) of what is stored, so a partial run that reaches finalize cannot wipe the catalog. Skips are logged as `sync.cleanup.skipped` with the counts. The same logic puts a contract on `Tapgoods_API_Client::get_inventories_from_graph()`: a page that could not be read must come back as `false`, which aborts the run before finalize, and never as a null or empty page, which the run reads as "this location has no more items". GraphQL reports failures as HTTP 200 with an `errors` array and `data: null`, so a status check alone is not enough (`ApiClientInventoriesTest`).
